...
//
    val result=predicted.withColumn("memberId",spls('memberId).cast("long").cast("string"))
      .select('memberId,'predict,when('predict==="3","7")
          .when('predict==="0","6")
          .when('predict==="1","5")
          .when('predict==="6","4")
        .when('predict==="4","3")
          .when('predict==="2","2")
          .when('predict==="5","1")as("用户价值"))
      .drop("rencency","frequency","moneyTotal","feature","predict")
//
    def catalogWrite =
      s"""{
